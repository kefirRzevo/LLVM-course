...
class IRGenerator final
  ...
  llvm::Function *curFunc_ = nullptr;
	...
	llvm::IntegerType* getLLVMType(IntType *) {
		return builder_->getInt32Ty();
	}

	llvm::Type* getLLVMType(VoidType* ) {
  ...
  llvm::Type *getLLVMType(Type *type) {
	...
	llvm::Value* callMemcpy(llvm::Value* dest, llvm::Value* src, ArrayType* type) {
		auto elemType = type->getElemType();
		auto llvmElemType = getLLVMType(elemType);
		...
		llvm::Function *memcpy = llvm::Intrinsic::getDeclaration(module_.get(), llvm::Intrinsic::memcpy, argsType);
...
public:
  ...
  IRGenerator(std::string_view filepath, llvm::LLVMContext &ctx)
      : context_(ctx),
				module_(std::make_unique<llvm::Module>(filepath, ctx)),
	...
	llvm::Function* getMainFunc() const {
		return mainFunc_;
	}
  ...
  llvm::Value* visit(InitListExpr &node) {
		...
		auto ret = new llvm::GlobalVariable(*module_, llvmType, true, llvm::GlobalValue::PrivateLinkage, constant);
		return ret;
	}
  ...
  llvm::Value* visit(FunctionDecl &node) {
		...
		auto llvmFunc = llvm::Function::Create(llvmType, llvm::Function::ExternalLinkage, name, module_.get());
		curFunc_ = llvmFunc;
