...
Driver::Driver(std::string_view moduleName)
    : moduleName_(moduleName), astBuilder_(std::make_unique<ASTBuilder>()),
...
FunctionType *Driver::getFunctionType(Type *retType,
                                      std::vector<Type *> &&paramTypes) {
  return typeCreator_->getType<FunctionType>(retType, std::move(paramTypes));
}

void Driver::analyze() { ::paracl::semanticAnalyze(*this); }

void Driver::execute() { ::paracl::execute(moduleName_, getRoot()); }
