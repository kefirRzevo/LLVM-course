...
ExternFunctionDecl *getLoadedFunction(Driver &driver, std::string_view name,
                                      FunctionType *type, void *ptr) {
...
ExternFunctionDecl *getGuiQuitEvent(Driver &driver) {
  ...
  auto retType = driver.getIntType();
  auto funcType = driver.getFunctionType(retType, std::move(params));
  return getLoadedFunction(driver, "gui_quit_event", funcType,
                           (void *)&gui_quit_event);
}
...
ExternFunctionDecl *getGuiSetPixel(Driver &driver) {
  ...
  auto retType = driver.getVoidType();
  auto funcType = driver.getFunctionType(retType, std::move(params));
  return getLoadedFunction(driver, "gui_set_pixel", funcType,
                           (void *)&gui_set_pixel);
}
